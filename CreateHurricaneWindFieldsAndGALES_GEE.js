...
hgt_array = hgt_array.updateMask(hgt_array.neq(-9999)).multiply(100).divide(2.54).divide(12);//Convert to ft
...
function GALES(WindSpeed, Hgt, CrownDepth, Spacing, ModRupture){
  ...
  function GALESFun(){
    ...
    var G35 = ee.Image(1).subtract(l.multiply(15).sqrt().multiply(-1).exp().multiply(-1)).divide(l.multiply(15).sqrt());
...
function createHurricaneWindFieldsWrapper(rows){
  ...
  Map.addLayer(wind_array,{min:75,max:160,palette:palettes.niccoli.isol[7]},'Max Wind',false);
  var GALESOut = GALES(wind_array.multiply(0.447), hgt_array.multiply(0.1), hgt_array.multiply(0.33*0.1), 5.0, 8500);
  Map.addLayer(GALESOut,{min:0,max:100,palette:palettes.niccoli.isol[7]}, 'GALES');
